...
class SignaturePad extends Component {
  ...
  _parseMessageFromWebViewNavigationChange = (newUrl) => {
    ...
    let hashUrlIndex = newUrl.lastIndexOf('#')
    if (hashUrlIndex === -1) {
      return
    }
    ...
    while (parameterMatch) {
      let parameterPair = parameterMatch[1] //For example executeFunction=jsError or arguments=...
  ...
  _attemptToExecuteNativeFunctionFromWebViewMessage = (message) => {
    if (message.executeFunction && message.arguments) {
      let parsedArguments = JSON.parse(message.arguments)
  ...
  onMessage = (event) => {
    let base64DataUrl = JSON.parse(event.nativeEvent.data)
